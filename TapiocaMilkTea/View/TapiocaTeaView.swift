...
struct TapiocaTeaView: View {
    ...
    var body: some View {
        let fl: Flavor = Flavor(rawValue: Int(flavor))!
        let fColors: [Color] = TapiocaTeaView.makeTeaGradient(fl)
        let teaGradient: LinearGradient = LinearGradient(
            gradient: .init(colors: fColors),
            startPoint: .top,
            endPoint: .bottom
        )
        
        let ic: IceCream = IceCream(rawValue: Int(iceCream))!
        let iColors: [Color] = TapiocaTeaView.makeIceGradient(ic)
        let iceGradient: LinearGradient = LinearGradient(
                ...
                ForEach(nataDeCocoArray) { coco in
                    NataDeCocoShape(coco: coco)
                        ...
                        .rotationEffect(
                            ...
                            anchor: .init(x: (coco.pt.x + Coco.size / 2),
...
struct IceCreamShape: Shape {
    func path(in rect: CGRect) -> Path {
        Path { path in
            let scale = min(rect.width, rect.height)
            path.addArc(
                center: CGPoint(x: scale * 0.50, y: scale * 0.30),
                radius: scale * 0.26,
    ...
    func path(in rect: CGRect) -> Path {
        Path { path in
            ...
            path.addLines([
                ...
                CGPoint(x: scale * 0.3000, y: scale * 1.00),
                CGPoint(x: scale * 0.7000, y: scale * 1.00),
...
struct TapiocaShape: Shape {
    ...
    func path(in rect: CGRect) -> Path {
        Path { path in
            let scale = min(rect.width, rect.height)
            path.addArc(
                center: CGPoint(x: scale * tapioca.pt.x, y: scale * tapioca.pt.y),
...
fileprivate let tapiocaArray: [Tapioca] = [
...
fileprivate let nataDeCocoArray: [Coco] = [
...
struct TapiocaTeaView_Previews: PreviewProvider {
    static var previews: some View {
        TapiocaTeaView(
            iceCream: Int(IceCream.Tea.rawValue),
